...
export default function NotesApplicationView(props: WindowProps) {
  ...
  function createOnSave() {
    const path = '/Users/abeih/Desktop/';

    const desktopDirectory = fs.getDirectory(path);
    if (!desktopDirectory.ok) { return; }

    const template = t('filesystem.new_file');
    const title = generateUniqueNameForDirectory(desktopDirectory.value, template);
    ...
    application.compositor.prompt(windowContext.id, t('notes.create_file_instructions'), title)
      .then((title) => {
        ...
        const textFile = fs.addTextFile(desktopDirectory.value, title, content, true);
