...
type FolderViewAppearance = 'default' | 'finder';
...
type FolderViewProps = {
  ...
  appearance?: FolderViewAppearance
}
...
export function FolderView(props: FolderViewProps) {
  const { directory, apis, onFileOpen, localIconPosition, allowOverflow: propOverflow, appearance: propAppearance } = props;
  const fs = apis.fileSystem;

  const useLocalIconPosition = localIconPosition ?? false;
  const allowOverflow = propOverflow ?? true;
  const appearance: FolderViewAppearance = propAppearance ?? 'default';
  ...
  const fileDraggingCurrentNode = useRef<Element | undefined>(undefined);
  const fileDraggingSession = useRef<DragAndDropSession | null>(null);
  ...
  function onFileDraggingMove(evt: PointerEvent) {
    ...
    if (passesDraggingThreshold && !isDragging.current) {
      ...
      // NOTE(Abeih): Not sure why the scrollOffset{x/y} need to be multiplied by 2
  ...
  const icons = files.map((entry, index) => {
    return <FolderIcon key={index} folderIconEntry={entry} index={index} appearance={appearance} />
  });
