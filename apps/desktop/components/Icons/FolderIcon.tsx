...
type FolderIconAppearance = 'default' | 'finder';
...
export default function FolderIcon(props: { folderIconEntry: FolderIconEntry, index: number, appearance?: FolderIconAppearance }) {
  const { folderIconEntry: folderIconEntry, index, appearance = 'default' } = props;
  ...
  const finderTheme = appearance === 'finder';
  const containerClasses = [file.kind, styles.container, selected].join(' ');
  const textClasses = [styles.textContainer, finderTheme ? styles['finder-text'] : ''].join(' ');
