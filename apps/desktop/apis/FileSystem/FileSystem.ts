...
import { snakeConfig } from "@/applications/Snake/Snake";
import { memesConfig } from "@/applications/Memes/Memes";
...
export function createBaseFileSystem(): FileSystem {
  ...
  const snake = fileSystem.addApplication(snakeConfig);
  const memes = fileSystem.addApplication(memesConfig);
  ...
  const abeih = fileSystem.addDirectory(users, 'abeih', false, true);

  const desktop = fileSystem.addDirectory(abeih, 'Desktop', false, true);
  const trashCanIcon = { src: '/icons/trash-icon.png', alt: 'Trash can icon' };
  const trash = fileSystem.addDirectory(abeih, 'Trash', false, true, trashCanIcon);

  fileSystem.addHyperLink(desktop, applications, 'Applications', applicationFolderIcon, true);
  fileSystem.addImage(desktop, 'THE-GOAT', '.png', '/images/the-goat.png', 'The GOAT himself', true);

  if (snake.ok) {
    const snakeShortcutIcon = { src: '/icons/snake-game.png', alt: 'Snake' };
    fileSystem.addHyperLink(desktop, snake.value, 'Snake', snakeShortcutIcon, true);
  }
  if (memes.ok) {
    const memesShortcutIcon = { src: '/icons/decent-memes.png', alt: 'Memes' };
    fileSystem.addHyperLink(desktop, memes.value, 'Memes', memesShortcutIcon, true);
  }
...
function entriesWithinSelection(entries: Point[], x: number, y: number, dimensions: { width: number, height: number }): number {
  ...
  // NOTE(Abeih): The width & height are used for both the new incoming entry as the already existing entries.
